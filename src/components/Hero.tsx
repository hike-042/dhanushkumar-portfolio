...
export default function Hero() {
  ...
  const blob1CY = useTransform(() => blob1Y.get() + b1my.get())
  const blob2CY = useTransform(() => blob2Y.get() + b2my.get())
  const blob3CY = useTransform(() => blob3Y.get() + b3my.get())
  ...
  return (
    ...
    >
      ...
      >
        <motion.div variants={imgVariant} className="hero-img-wrap" style={{ flexShrink: 0, y: imgY, willChange: 'transform' }}>
          <div className="profile-ring-wrap">
            <div style={{ width: 'clamp(150px, 20vw, 220px)', height: 'clamp(150px, 20vw, 220px)', borderRadius: '50%', overflow: 'hidden', boxShadow: '0 8px 40px rgba(0,0,0,0.22)' }}>
              <Image src="/profile-full.jpg" alt="Dhanush Kumar R" width={220} height={220} style={{ objectFit: 'cover', width: '100%', height: '100%' }} priority />
            </div>
          </div>
        </motion.div>
        ...
        <motion.div style={{ flex: 1, minWidth: 280, y: textY, willChange: 'transform' }}>
          ...
          <motion.h1 variants={block} className="font-fraunces" style={{ fontSize: 'clamp(2.8rem, 7vw, 5.5rem)', fontWeight: 400, lineHeight: 0.96, letterSpacing: '-.03em', marginBottom: 12 }}>
            <TextScramble text="Dhanush" delay={300} duration={1000} />
            <br />
            <TextScramble text="Kumar R" delay={600} duration={1000} className="gradient-text" style={{ fontStyle: 'italic' }} />
          </motion.h1>

          <motion.p variants={block} className="font-fraunces" style={{ fontSize: 'clamp(1rem, 2vw, 1.15rem)', fontWeight: 300, color: 'var(--text-2)', fontStyle: 'italic', marginBottom: 16 }}>
            aka Arco · PES University Bengaluru · MBA HR &apos;26
          </motion.p>

          <motion.div variants={block} style={{ height: 28, overflow: 'hidden', marginBottom: 28, position: 'relative' }}>
            <AnimatePresence mode="wait">
              <motion.span
                ...
                style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: '.92rem', fontWeight: 500, color: 'var(--accent)', letterSpacing: '.02em', position: 'absolute', whiteSpace: 'nowrap' }}
              >
                <motion.span animate={{ x: [0, 4, 0] }} transition={{ duration: 0.6, delay: 0.2 }} style={{ fontSize: '.8rem', opacity: 0.7 }}>
                  →
                </motion.span>
                {ROLES[roleIdx]}
          ...
          <motion.div variants={block} style={{ maxWidth: 540, marginBottom: 36 }}>
            ...
            <p style={{ fontSize: '1rem', fontWeight: 300, color: 'var(--text-2)', lineHeight: 1.8 }}>
              I combine a Computer Science degree with an MBA in HR to recruit smarter, understanding both what an engineering manager needs and what makes a candidate exceptional. That&apos;s a rare combination in Talent Acquisition, and I use it every day.
            </p>
          </motion.div>

          <motion.blockquote variants={block} style={{ fontSize: '1rem', fontWeight: 300, color: 'var(--text-3)', lineHeight: 1.7, borderLeft: '2px solid var(--accent)', paddingLeft: 16, marginBottom: 40, fontStyle: 'italic', maxWidth: 460 }}>
            &ldquo;Everyone is exceptional in their own way. They just need the right opportunity to write their story.&rdquo;
          </motion.blockquote>

          <motion.div variants={block} className="hero-cta-row" style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 52 }}>
            <MagneticButton href="#experience" style={{ padding: '15px 34px', background: 'var(--accent)', color: '#fff', borderRadius: 8, fontSize: '.95rem', fontWeight: 500, letterSpacing: '.02em' }}>
              View My Work
            </MagneticButton>
          ...
          <motion.div variants={block} style={{ display: 'flex', gap: 0, flexWrap: 'wrap', borderTop: '1px solid var(--line)', paddingTop: 28 }}>
            {[
              ...
              <div key={stat.label} style={{ paddingRight: 28, marginRight: 28, borderRight: i < 3 ? '1px solid var(--line)' : 'none', marginBottom: 8 }}>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.2, duration: 0.8 }} className="scroll-hint" style={{ position: 'absolute', bottom: 28, left: '50%', transform: 'translateX(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, zIndex: 1, pointerEvents: 'none' }}>
        <span style={{ fontSize: '.6rem', color: 'var(--text-3)', letterSpacing: '.14em', textTransform: 'uppercase', fontWeight: 400 }}>
          scroll
        </span>
        <motion.div animate={{ y: [0, 7, 0] }} transition={{ duration: 1.8, repeat: Infinity, ease: 'easeInOut' }} style={{ width: 1.5, height: 32, background: 'linear-gradient(to bottom, var(--accent), transparent)', borderRadius: 2 }} />
      </motion.div>
    </section>
  )
}
